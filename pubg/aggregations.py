...
def compute_top_mates(conn, my_account_id: str,
    ...
    for r in rows:
        ...
        # Synergy-Score: Team-KDA × Win-Rate-Bonus × Stabilitäts-Faktor.
        # Stabilität dämpft Glücks-Werte mit wenig Matches:
        #   2 Matches → Faktor 0.17 · 5 → 0.33 · 11 → 0.52 · 50 → 0.83
        # Damit ist 9.0 Synergy in 2 Spielen niedriger als 2.5 in 11 Spielen.
        stability = shared / (shared + 10.0)
        synergy_raw = ((my_kd + mate_kd) / 2.0) * (1.0 + win_rate / 100.0)
        synergy = synergy_raw * stability
        out.append({
